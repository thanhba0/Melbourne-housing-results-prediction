...
class App(Frame):
    ...
    def __init__(self, master):
        ...
        self.btn_predict_sample = Button(Frame_top_3, text='Predict sample', command=self.predict_sample)
        self.btn_predict_sample.pack(fill = "both", pady=3)
        # botttt
    ...
    def predict_sample(self):
        ...
        window.title('Input a sample')
        names = list(self.data.origin_df.columns )
        if 'Type' in names:
            names.remove('Type')
        entries = []
        frame = Frame(window)
        ...
        for i in range(len(names)):
            ...
            if i < len(names)/2 :
                f1 = Frame(frame)
            else: f1 = Frame(frame_1)
            f1.pack(side=LEFT)   
            Label(f1, 
                  text='{} ({})'.format(col, self.data.origin_df[col].dtype),
                  width = 20,
        ...
        def summit():
            ...
            for i in range(len(entries)):
                ...
                if  self.data.origin_df[col].dtype =='object':
                    d[col] = [val]
                    types[col] = str
                else: 
                    self.data.origin_df[col].astype(float, copy=False)
                    types[col] = float
                    if val =='':
                        d[col] = [np.NaN]
                    else:
            ...
            df.astype(types,copy=False)
            X = self.data.predict(df)

            label = self.model.predict(X)
            self.label1['text'] = 'Predict: {} \n'.format(label)

        btn = Button(window, text='OK', width=40, fg='white', bg='Green', command=summit)
        btn.pack()
