...
class dataloader():
    ...
    def transform(self):
        ...
        self.labels.apply(lambda x: self.class_name.index(x))
    ...
    def predict(self, df):
        ...
        if 'Type' in col_object:
            col_object = col_object.drop('Type')

        # Impute data
        df[col_object] = self.imputer_object.transform(df[col_object])
        df[col_numeric] = self.imputer_numeric.transform(df[col_numeric])
        
        if len(df) == 1:
            for i in df:
                if df[i][0] == '' and df[i].dtype == 'object':

                    arr = self.origin_df[i].describe().unique()
                    df[i][0] = arr[2]
